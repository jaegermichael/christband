          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {books.map((book) => (
              <div key={book.title} className="group rounded-2xl border border-[#E8E0D0] bg-[#FFFFFF] shadow-sm transition-all hover:shadow-lg hover:border-[#D4AF37]/50 overflow-hidden">
                ...
                <div className="flex h-52 flex-col items-center justify-center bg-gradient-to-br from-[#551839] to-[#3B1027] p-4 text-center">
                ...
                <div className="p-5">
                  ...
                  <h3 className="mt-2 font-serif font-bold text-[#2F0B20] leading-snug">{book.title}</h3>
                  <div className="mt-1 flex items-center gap-1.5 text-xs text-[#7A5A6D]">
                  ...
                  <Link href={`/cart?item=${encodeURIComponent(book.title)}`} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#551839] to-[#7A2A5E] py-2.5 text-sm font-medium text-[#FFFDF7] transition-all hover:brightness-110">
