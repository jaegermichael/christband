...
const plans = [
  {
    ...
    ctaStyle: "border border-[#551839] text-[#551839] hover:bg-[#F5F0E8]",
  },
  {
    name: "Standard",
    price: "$5",
    ...
    cta: "Subscribe Now",
    popular: true,
    gradient: "from-[#551839] to-[#7A2A5E]",
    borderColor: "border-[#D4AF37]",
    ctaStyle: "bg-gradient-to-r from-[#D4AF37] to-[#E8CC6A] text-[#2F0B20] hover:brightness-110",
  },
  {
    name: "Premium",
    price: "$15",
    ...
    gradient: "from-[#F5F0E8] to-[#FFFFFF]",
    borderColor: "border-[#E8E0D0]",
    ctaStyle: "bg-gradient-to-r from-[#551839] to-[#7A2A5E] text-[#FFFDF7] hover:brightness-110",
  },
]
      ...
      <main className="flex-1">
        ...
        <section className="mx-auto max-w-6xl px-4 py-12">
          <div className="grid gap-8 md:grid-cols-3">
            {plans.map((plan) => (
              ...
              >
                {plan.popular && (
                  <div className="bg-gradient-to-r from-[#D4AF37] to-[#E8CC6A] px-4 py-1.5 text-center text-xs font-bold uppercase tracking-widest text-[#2F0B20]">
                    Most Popular
                  </div>
                )}

                <div className={`bg-gradient-to-br ${plan.gradient} p-6 ${plan.popular ? "text-[#FFFDF7]" : "text-[#2F0B20]"}`}>
                  <div className="mb-4 flex items-center gap-3">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${plan.popular ? "bg-[#D4AF37]/20" : "bg-[#551839]/10"}`}>
                      <plan.icon className={`h-6 w-6 ${plan.popular ? "text-[#D4AF37]" : "text-[#551839]"}`} />
                    </div>
                  ...
                  <div className="mb-2 flex items-baseline gap-1">
                    ...
                    <span className={`text-sm ${plan.popular ? "text-[#D8B9CB]" : "text-[#7A5A6D]"}`}>/{plan.period}</span>
                  </div>
                  <p className={`text-sm leading-relaxed ${plan.popular ? "text-[#D8B9CB]" : "text-[#7A5A6D]"}`}>
                    {plan.description}
                  </p>
                </div>
                ...
                <div className="flex flex-1 flex-col bg-[#FFFFFF] p-6">
                  <ul className="mb-6 flex flex-1 flex-col gap-3">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-2.5 text-sm text-[#2F0B20]">
        ...
        <section className="bg-[#F5F0E8] py-12">
          <div className="mx-auto max-w-4xl px-4 text-center">
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold text-[#2F0B20]">Zimbabwe Payment Methods</h2>
            <p className="mx-auto mt-3 max-w-lg text-[#7A5A6D] leading-relaxed">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {paymentMethods.map((method) => (
                ...
                >
                  <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-[#551839] to-[#7A2A5E]">
                    <Smartphone className="h-7 w-7 text-[#D4AF37]" />
                  </div>
                  <h3 className="font-serif text-lg font-bold text-[#2F0B20]">{method.name}</h3>
                  <p className="mt-1 text-sm text-[#7A5A6D]">{method.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mx-auto max-w-3xl px-4 py-12">
          <h2 className="mb-8 text-center font-serif text-3xl font-bold text-[#2F0B20]">Frequently Asked Questions</h2>
          <div className="flex flex-col gap-4">
            {[
              ...
              >
                <h3 className="font-serif text-lg font-semibold text-[#2F0B20]">{faq.q}</h3>
                <p className="mt-2 text-sm text-[#7A5A6D] leading-relaxed">{faq.a}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* CTA */}
        <section className="bg-gradient-to-br from-[#551839] via-[#3B1027] to-[#2F0B20] py-16">
          <div className="mx-auto max-w-3xl px-4 text-center">
            ...
            <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <Link
                href="/membership"
                className="rounded-xl bg-gradient-to-r from-[#D4AF37] to-[#E8CC6A] px-8 py-3.5 text-sm font-bold text-[#2F0B20] shadow-lg transition-all hover:brightness-110"
              >
                Register Now
              </Link>
              <Link
