...
const stats = [
  { label: "Total Members", value: "247", change: "+18 this week", icon: Users, color: "from-[#551839] to-[#7A2A5E]", href: "/admin/members" },
  { label: "Churches", value: "52", change: "+3 this month", icon: Church, color: "from-[#3B1027] to-[#551839]", href: "/admin/churches" },
  { label: "Pastors", value: "68", change: "+5 this month", icon: UserCheck, color: "from-[#7A2A5E] to-[#9B4E79]", href: "/admin/pastors" },
  { label: "Active Ads", value: "23", change: "6 pending", icon: Megaphone, color: "from-[#D4AF37] to-[#C49B2F]", href: "/admin/adverts" },
  { label: "Pending Approvals", value: "35", change: "Needs attention", icon: Clock, color: "from-[#C49B2F] to-[#A88525]", href: "/admin/members" },
  { label: "Payments", value: "$1,240", change: "4 to confirm", icon: CreditCard, color: "from-[#551839] to-[#3B1027]", href: "/admin/payments" },
]
      ...
      <div className="p-6 lg:p-8">
            ...
            return (
              ...
              >
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${stat.color} shadow-md`}>
                  <Icon className={`h-6 w-6 ${isGold ? "text-[#2F0B20]" : "text-[#FFFDF7]"}`} />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-[#7A5A6D]">{stat.label}</p>
                  <p className="mt-1 font-serif text-2xl font-bold text-[#2F0B20]">{stat.value}</p>
                  <p className="mt-0.5 text-xs text-[#A48295]">{stat.change}</p>
                </div>
              </Link>
            )
          })}
        ...
        <div className="mt-8">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-serif text-xl font-bold text-[#2F0B20]">Recent Activity</h2>
            <span className="text-sm text-[#7A5A6D]">Last 24 hours</span>
          </div>
              ...
              {recentActivity.map((item, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#2F0B20]">
                      {item.action}:{" "}
                      <span className="font-semibold">{item.name}</span>
                    </p>
                    <p className="text-xs text-[#A48295]">{item.city}</p>
                  </div>
                  <span
                  ...
                  <span className="shrink-0 text-xs text-[#A48295]">{item.time}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#551839]/10">
              <Users className="h-5 w-5 text-[#551839]" />
            </div>
            <div>
              <p className="text-sm font-semibold text-[#2F0B20]">Review Members</p>
              <p className="text-xs text-[#7A5A6D]">12 pending approvals</p>
            </div>
          </Link>
          <Link
            href="/admin/adverts"
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-[#2F0B20]">Review Adverts</p>
              <p className="text-xs text-[#7A5A6D]">6 pending review</p>
            </div>
          </Link>
          <Link
            href="/admin/payments"
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-[#2F0B20]">Confirm Payments</p>
              <p className="text-xs text-[#7A5A6D]">4 awaiting confirmation</p>
            </div>
          </Link>
        </div>
      </div>
