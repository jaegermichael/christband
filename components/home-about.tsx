...
export function HomeAbout() {
  ...
  return (
    <section className="border-y border-primary/10 bg-[#F7F2FA] py-20 md:py-28" aria-labelledby="about-heading">
      <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.28em] text-accent">Why ChristBand</p>
        ...
        <div>
          <div className="relative overflow-hidden rounded-xl border border-primary/10 bg-white p-2 shadow-[0_22px_60px_rgba(58,3,83,0.1)]">
            <Image src={encodeURI("/images/Christian Group Prayer – African Faith, Unity & Spiritual Growth Inspiration.jpg")} alt="A Christian group gathered in prayer" width={800} height={400} className="h-72 w-full rounded-lg object-cover md:h-96" />
            <div className="pointer-events-none absolute inset-2 rounded-lg bg-gradient-to-t from-[#3A0353]/80 via-transparent to-transparent" />
            <p className="absolute bottom-7 left-7 max-w-xs font-serif text-2xl leading-tight text-white">United in the Spirit, ready to serve.</p>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            {values.map(({ icon: Icon, title, desc }) => <div key={title} className="border-t border-primary/15 pt-4"><Icon className="h-5 w-5 text-accent" aria-hidden="true" /><h3 className="mt-3 text-sm font-semibold text-foreground">{title}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{desc}</p></div>)}
          </div>
        </div>
      </div>
    </section>
