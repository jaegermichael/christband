...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-[#3A0353]/95 backdrop-blur-xl">
      <div className="border-b border-white/10 bg-[#4F1A74] text-xs text-white/80">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-3" aria-label="Main navigation">
        <Link href="/" className="group flex shrink-0 items-center gap-2" onClick={closeMobile}>
          ...
          <span>
            <span className="block font-serif text-xl font-bold tracking-tight text-white group-hover:text-secondary">ChristBand</span>
            ...
            link.children ? (
              <div key={link.label} className="relative" onMouseEnter={() => setOpenDropdown(link.label)} onMouseLeave={() => setOpenDropdown(null)}>
                <Link
                  ...
                  className="flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-white/90 hover:bg-white/10 hover:text-secondary"
                >
                  {link.label}
                ...
                {openDropdown === link.label && (
                  <div className="absolute left-0 top-full z-50 min-w-[210px] rounded-lg border border-secondary/25 bg-[#211132] p-2 shadow-2xl" role="menu">
                    {link.children.map((child) => (
                      <Link key={child.href} href={child.href} role="menuitem" className="block rounded-md px-3 py-2 text-sm text-white/85 hover:bg-secondary/10 hover:text-secondary">
                        {child.label}
                      </Link>
                    ))}
                  </div>
              ...
              <Link key={link.href} href={link.href} className="rounded-md px-3 py-2 text-sm font-medium text-white/90 hover:bg-white/10 hover:text-secondary">
                {link.label}
              </Link>
            ),
          )}
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="rounded-md border border-white/10 p-2 text-white hover:border-secondary/50 hover:text-secondary lg:hidden" aria-label="Toggle navigation menu" aria-expanded={mobileOpen} aria-controls="mobile-menu">
          ...
          {navLinks.map((link) =>
            link.children ? (
              <div key={link.label} className="border-b border-white/10">
                <button type="button" onClick={() => setOpenDropdown(openDropdown === link.label ? null : link.label)} aria-expanded={openDropdown === link.label} className="flex w-full items-center justify-between py-3 text-left text-sm font-semibold text-white">
                  {link.label}
                ...
                {openDropdown === link.label && (
                  <div className="mb-2 ml-3 border-l border-secondary/50 pl-3">
                    {link.children.map((child) => (
                      <Link key={child.href} href={child.href} onClick={closeMobile} className="block py-2 text-sm text-white/70 hover:text-secondary">{child.label}</Link>
                    ))}
                  </div>
                )}
              </div>
            ) : (
              <Link key={link.href} href={link.href} onClick={closeMobile} className="block border-b border-white/10 py-3 text-sm font-semibold text-white hover:text-secondary">{link.label}</Link>
            ),
          )}
